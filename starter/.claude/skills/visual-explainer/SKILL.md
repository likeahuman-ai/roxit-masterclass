...
**Design quality requirement.** Every HTML page must follow the `/frontend-design` plugin principles: distinctive typography (not Inter/Roboto/Arial), intentional color palette with CSS custom properties, 4px spacing rhythm, and visual hierarchy that makes the content immediately readable. Never produce generic dark-on-white boilerplate — every diagram should look like it was made by a designer, not a template engine.
...
**Output location:** Write to `/workspace/diagrams/`. Use a descriptive filename based on content: `architecture.html`, `pipeline-flow.html`, `schema-overview.html`. Create the directory if it doesn't exist.

**Open in browser:**
- macOS: `open /workspace/diagrams/filename.html`
- Linux: `xdg-open /workspace/diagrams/filename.html`
